...
def train(X, f, W1, W2, tol, N, m, ya, yb, stepsize, gamma):
    ...
    while (np.linalg.norm(dW1) > tol) & (np.linalg.norm(dW2) > tol):
        ...
        beta = np.random.rand()
        a = 1
        lossnow, _ = compute_loss(X, W1, W2, N, m, stepsize, f, ya, yb, gamma)
        lossnext, _ = compute_loss(X, W1-a*dW1, W2-a*dW2, N, m, stepsize, f, ya, yb, gamma)
        ...
        # Print loss
        if (k % 100 == 0):
            print(f'Iteration {k} - Loss: {lossnow:4f}\n'
