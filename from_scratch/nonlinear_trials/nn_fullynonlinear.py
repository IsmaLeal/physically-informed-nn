...
def compute_loss(X, W1, W2, N, m, stepsize, f, ya, typea, yb, typeb, gamma):
    ...
    f_vals = np.array([f(X[1, i], y_hat[0, i], y_1st[0, i]) for i in range(1, X.shape[1] - 1)])
...
def back_propagation(X, W1, W2, N, m, stepsize, f, ya, typea, yb, typeb, gamma):
    ...
    f_vals = np.array([f(X[1, i], y_hat[0, i], y_1st[0, i]) for i in range(1, N)])
    ...
    for j in range(m):  # For each bias
        ...
        _, y_1st_b1, _ = compute_loss(X, W1_b1, W2, N, m, stepsize, f, ya, typea, yb, typeb, gamma)
        f_b1 = np.array([f(X[1, i], y_b1[0, i], y_1st_b1[0, i]) for i in range(1, N)])
    ...
    for j in range(m):  # For each weight
        ...
        _, y_1st_w1, _ = compute_loss(X, W1_w1, W2, N, m, stepsize, f, ya, typea, yb, typeb, gamma)
        f_w1 = np.array([f(X[1, i], y_w1[0, i], y_1st_w1[0, i]) for i in range(1, N)])
        # Obtain the necessary array
        matrix_w1[j, :] -= f_w1
    # df/dw1
    ...
    _, y_1st_b2, _ = compute_loss(X, W1, W2_b2, N, m, stepsize, f, ya, typea, yb, typeb, gamma)
    f_b2 = np.array([f(X[1, i], y_b2[0, i], y_1st_b2[0, i]) for i in range(1, N)])
    matrix_b2 -= f_b2
    # df/db2
    ...
    for j in range(m):  # For each weight
        ...
        _, y_1st_w2, _ = compute_loss(X, W1, W2_w2, N, m, stepsize, f, ya, typea, yb, typeb, gamma)
        f_w2 = np.array([f(X[1, i], y_w2[0, i], y_1st_w2[0, i]) for i in range(1, N)])
        # Obtain the necessary array
        matrix_w2[j, :] -= f_w2
    # df/dw2
...
def train(X, f, W1, W2, tol, N, m, ya, typea, yb, typeb, stepsize, gamma):
    ...
    while lossnow > tol:
        ...
        # Next iteration
        k += 1
    print(f'{k} iterations to reach a loss of: {lossnow}')
